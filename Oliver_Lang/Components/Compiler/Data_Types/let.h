...
namespace Olly {
    ...
    public:
        ...
        let                pair()                                          const;  // Retreive the element pair of a collation.
        let               right()                                          const;  // Get the right hand element of an binary branch.
        let                left()                                          const;  // Get the left hand element of an binary branch.
        int_type         height()                                          const;  // Get the height of a branch.  

        let            set_pair(const let& other)                          const;
        let            set_left(const let& other)                          const;
        let           set_right(const let& other)                          const;
        let          set_height(const int_type& height)                    const;
    ...
    private:
        ...
        struct interface_type {
            ...
            virtual let             _right()                                        const = 0;
            virtual let             _left()                                         const = 0;
            virtual int_type        _height()                                       const = 0;

            virtual let             _set_pair(const let& other)                     const = 0;
            virtual let             _set_right(const let& other)                    const = 0;
            virtual let             _set_left(const let& other)                     const = 0;
            virtual let             _set_height(const int_type& other)              const = 0;
        ...
        struct data_type : interface_type {
            ...
            let             _right()                                        const;
            let             _left()                                         const;
            int_type        _height()                                       const;

            let             _set_pair(const let& other)                     const;
            let             _set_right(const let& other)                    const;
            let             _set_left(const let& other)                     const;
            let             _set_height(const int_type& other)              const;
    ...
    template<typename T>            /****   A Pair Of Objects  ****/
    ...
    template<typename T>            /****  Right Branch Of  ****/
    let _right_(const T& self);

    template<typename T>
    inline let _right_(const T& self) {
        return nothing();
    }


    template<typename T>            /****  Left Branch Of  ****/
    let _left_(const T& self);

    template<typename T>
    inline let _left_(const T& self) {
        return nothing();
    }


    template<typename T>            /****  The Height Of A Branch  ****/
    int_type _height_(const T& self);

    template<typename T>
    inline int_type _height_(const T& self) {
        return 0;
    }


    template<typename T>            /****   Set A Pair Of Objects  ****/
    let _set_pair_(const T& self, const let& other);

    template<typename T>
    inline let _set_pair_(const T& self, const let& other) {
        return nothing();
    }


    template<typename T>            /****  Set Right Branch Of  ****/
    let _set_right_(const T& self, const let& other);

    template<typename T>
    inline let _set_right_(const T& self, const let& other) {
        return nothing();
    }


    template<typename T>            /****  Set Left Branch Of  ****/
    let _set_left_(const T& self, const let& other);

    template<typename T>
    inline let _set_left_(const T& self, const let& other) {
        return nothing();
    }


    template<typename T>            /****  Set The Height Of A Branch  ****/
    let _set_height_(const T& self, const int_type& other);

    template<typename T>
    inline let _set_height_(const T& self, const int_type& other) {
        return 0;
    }
    ...
    inline let let::right() const {
        return _self->_right();
    }

    inline let let::left() const {
        return _self->_left();
    }

    inline int_type let::height() const {
        return _self->_height();
    }

    inline let let::set_pair(const let& other) const {
        return _self->_set_pair(other);
    }

    inline let let::set_right(const let& other) const {
        return _self->_set_right(other);
    }

    inline let let::set_left(const let& other) const {
        return _self->_set_left(other);
    }

    inline let let::set_height(const int_type& height) const {
        return _self->_set_height(height);
    }
    ...
    template <typename T>
    inline let let::data_type<T>::_pair() const {
        return _pair_(_data);
    }

    template <typename T>
    inline let let::data_type<T>::_left() const {
        return _left_(_data);
    }

    template <typename T>
    inline let let::data_type<T>::_right() const {
        return _right_(_data);
    }

    template <typename T>
    inline int_type let::data_type<T>::_height() const {
        return _height_(_data);
    }

    template <typename T>
    inline let let::data_type<T>::_set_pair(const let& other) const {
        return _set_pair_(_data, other);
    }

    template <typename T>
    inline let let::data_type<T>::_set_left(const let& other) const {
        return _set_left_(_data, other);
    }

    template <typename T>
    inline let let::data_type<T>::_set_right(const let& other) const {
        return _set_right_(_data, other);
    }

    template <typename T>
    inline let let::data_type<T>::_set_height(const int_type& height) const {
        return _set_height_(_data, height);
    }
