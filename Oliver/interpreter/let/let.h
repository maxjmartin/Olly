...
namespace Olly {
    ...
    let  first(let p);                        // Return the first element of an object.
    let second(let p);                        // Return the second element of an object.
    let  third(let p);                        // Return the third element of an object.
    ...
    template<typename T>
    inline let _shift_lead_(const T& self) {
        return nothing();
    }
    ...
    template<typename T>
    inline let _shift_last_(const T& self) {
        return nothing();
    }
    ...
    inline let let::shift_lead() const {
    ...
    inline let let::shift_last() const {
    ...
    template <typename T>
    inline let let::data_type<T>::_shift_lead() const {
    ...
    template <typename T>
    inline let let::data_type<T>::_shift_last() const {
    ...
    str_type to_lower(str_type str) {
    ...
    str_type to_upper(str_type str) {
    ...
    void ltrim(str_type& s) {
        if (s.empty()) {
            return;
        }
        s.erase(s.begin(), std::find_if(s.begin(), s.end(), [](int_type ch) {
            return !std::isspace(ch);
            }));
    }

    void rtrim(str_type& s) {
        if (s.empty()) {
            return;
        }
        s.erase(std::find_if(s.rbegin(), s.rend(), [](int_type ch) {
            return !std::isspace(ch);
            }).base(), s.end());
    }

    void lrtrim(str_type& s) {
        ltrim(s);
        rtrim(s);
    }

    str_type left_trim(str_type s) {
        ltrim(s);
        return s;
    }

    str_type right_trim(str_type s) {
        rtrim(s);
        return s;
    }
