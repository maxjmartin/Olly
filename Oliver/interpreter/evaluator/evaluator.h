...
namespace Olly {
	namespace eval {
		...
		private:
			...
			void function_operators(OP_CODE& opr);
		...
		evaluator::evaluator() : _variables(), _stack(), _code(1, expression()), _max_stack_size(DEFAULT_STACK_LIMIT) {
		}
			...
			do {
				...
				if (exp.type() == "expression") {
					...
					if (opr > OP_CODE::NOTHING_OP && opr < OP_CODE::END_OPERATORS_OP) {
						...
						if (opr < OP_CODE::FUNCTION_OPERATORS) {
							function_operators(opr);
						}
